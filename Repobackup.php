...
class RepoBackup {
	...
	function run() {
		...
		$line = $this->getInput( "Backup BitBucket v2 [y/n]:" );
		if ( strtolower( $line ) == 'y' ) {
			$user     = $this->getInput( "BitBucket client_id:" );
			$pass     = $this->getInput( "BitBucket client_secret:" );
			$team     = $this->getInput( "BitBucket Team:" );
			$bbSource = new BitbucketSourceV2();
			$bbSource->setCredentials( array( 'client_id' => $user, 'client_secret' => $pass, 'team' => $team ) );
			$this->backupRepositories( $bbSource, 'BitBucket/' . $user . '/' );
		}
	}
